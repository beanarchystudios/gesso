<script lang="ts">
	...
	const filtered = $derived.by(() => {
		...
		return announcements.filter((a) => {
			const hay = `${a.title} ${a.authorName} ${stripHtml(a.message)}`.toLowerCase();
...
<svelte:head>
	<title>Announcements | Gesso</title>
</svelte:head>
			...
			{:else if filtered.length === 0}
				<div class="rounded-xl border border-dashed px-6 py-16 text-center">
					...
					<p class="mt-3 text-sm font-medium">
						{query ? 'No matches' : 'No announcements'}
					</p>
					<p class="mt-1 text-sm text-muted-foreground">
						{query ? 'Try a different search term.' : 'This course has no announcements.'}
					</p>
					{#if query}
						<button
							type="button"
				...
				<div class="grid grid-cols-1 gap-3 xl:grid-cols-2 xl:gap-4 2xl:gap-5">
					{#each filtered as a (a.id)}
						<article class="rounded-xl border bg-card p-4">
							<h2 class="text-sm leading-snug font-semibold">{a.title}</h2>
							<p class="mt-1 text-xs text-muted-foreground">
								{a.authorName} · {formatDate(a.postedAt)}
								{#if a.replyCount}· {a.replyCount} repl{a.replyCount === 1 ? 'y' : 'ies'}{/if}
							</p>
							{#if stripHtml(a.message)}
								<p class="mt-3 line-clamp-3 text-sm leading-relaxed text-foreground/80">
									{stripHtml(a.message).slice(0, 280)}
								</p>
							{/if}
							{#if a.htmlUrl}
								<a
